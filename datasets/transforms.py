...
def crop(image, target, region):
    ...
    if "boxes" in target or "masks" in target or "points" in target:
        # favor boxes selection when defining which elements to keep
        # this is compatible with previous implementation
        if "points" in target:
            cropped_points = target['points']
            keep = torch.all(((cropped_points > 0) * (cropped_points < max_size))[:, POINT_IDX, :], dim=1)
...
class RandomColorJitter(object):
    def __init__(self, prob=0.8):
        self.prob = prob
        self.color_jitter = T.ColorJitter(0.4, 0.4, 0.2, 0.1)
...
class RandomGrayScale(object):
    def __init__(self, prob=0.2):
        self.gary_scale = T.RandomGrayscale(p=prob)

    def __call__(self, image, target):
        image = self.gary_scale(image)
        return image, target
